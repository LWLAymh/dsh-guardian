...
/** Upper bound for persisted `before` content; larger files stay in-memory only. */
const MAX_PERSISTED_BEFORE_CHARS = 512 * 1024;

/** Per-session promise chain so concurrent persists cannot interleave/corrupt the JSON file. */
const persistQueues = new Map();

function persistFiles(sessionId) {
  const previous = persistQueues.get(sessionId) ?? Promise.resolve();
  const next = previous.then(() => doPersistFiles(sessionId)).catch(() => {});
  persistQueues.set(sessionId, next);
  return next;
}

async function doPersistFiles(sessionId) {
  try {
    ...
    const entries = [];
    for (const snap of files.values()) {
      if (typeof snap.before === "string" && snap.before.length > MAX_PERSISTED_BEFORE_CHARS) continue;
      entries.push({ path: snap.path, before: snap.before });
    }
    const data = { files: entries };
    await writeFile(undoFile(sessionId), JSON.stringify(data), { encoding: "utf8", mode: 0o600 });
  } catch {
...
function apply(ctx) {
  ...
  ctx.commands.register({
    ...
    async handler(invocation) {
      ...
      if (/[\r\n]/.test(invocation.rawInput)) {
        return { kind: "error", text: "Invalid path: newlines are not allowed." };
      }

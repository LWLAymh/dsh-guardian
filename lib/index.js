...
async function loadFiles(sessionId) {
  if (changesBySession.has(sessionId)) return changesBySession.get(sessionId);
  try {
    const raw = await readFile(undoFile(sessionId), "utf8");
    const data = JSON.parse(raw);
    if (!Array.isArray(data.files)) return changesFor({ session: { id: sessionId } });
    const map = new Map();
    ...
    // no persisted snapshot yet (or unreadable) — leave the in-memory map empty
  }
  ...
  if (commands !== undefined) {
    commands.register({
      ...
      async handler(invocation) {
        ...
        const files = await loadFiles(sessionId);
        if (files === undefined || files.size === 0) {

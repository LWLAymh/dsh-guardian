 ...
 *    `write`-created files (before === null) are removed again.
 */
...
/** Session-scoped {absolutePath -> {path, before}} records for undo. */
const changesBySession = new Map();

function changesFor(agent) {
  const id = agent.session.id;
  let files = changesBySession.get(id);
  if (files === undefined) {
    files = new Map();
    changesBySession.set(id, files);
  }
  return files;
}
  ...
  ctx.on("tools/result", (exec, result) => {
    try {
      ...
      if (existing === undefined) {
        files.set(path, { path, before: value.before });
      }
    } catch {
  ...
  if (commands !== undefined) {
    commands.register({
      ...
      async handler(invocation) {
        const files = changesBySession.get(invocation.agent.session.id);
        ...
        if (input === "" || input === "--all") {
          ...
          for (const path of paths) {
            ...
            try {
              ...
              return {
                kind: "error",
                text: `Failed to revert ${path}: ${error instanceof Error ? error.message : String(error)}`
              };
            }
          }
        ...
        const snap = files.get(input) ?? files.get(absolutePath(invocation.agent, input));
        if (snap === undefined) {
          return { kind: "error", text: `No recorded change for path: ${input}` };
        }
        try {
          await restoreFile(snap);
          files.delete(snap.path);

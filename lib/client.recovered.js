window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var css = [
      ...
      ".dshg_summary_files{display:flex;flex-direction:column;gap:6px}",
      ".dshg_summary_file{display:flex;flex-direction:column;gap:4px;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-specific-input-major);border-radius:10px;padding:6px 10px;min-width:0}",
    ...
    function FileChangeRow(props) {
      ...
      if (escalation) {
        children.push(React.createElement("span", { key: "perm", className: "dshg_perm_badge", title: "需要沙盒外权限: " + args.sandbox_permissions }, "需要权�? " + args.sandbox_permissions));
      }
      ...
      if (stats !== null && state !== "error") {
        ...
        if (added > 0 || removed > 0) {
          var statChildren = [];
          if (added > 0) statChildren.push(React.createElement("span", { key: "a", className: "dshg_add" }, "+" + added + "�?));
          if (removed > 0) statChildren.push(React.createElement("span", { key: "r", className: "dshg_rem" }, "-" + removed + "�?));
      ...
      if (model === null) {
        ...
        return React.createElement("div", { className: "dshg_card" },
          React.createElement("div", { className: "dshg_row", "data-state": state },
            ...
            permDir !== null
              ? React.createElement("span", { key: "perm", className: "dshg_perm_badge", title: "需要沙盒外权限: " + shellArgs.sandbox_permissions }, "需要权�? " + shellArgs.sandbox_permissions + " · " + permDir)
              : null,
    ...
    function computeUnkeptFiles(nodes, sessionId) {
      ...
      for (var i = 0; i < nodes.length; i++) {
        ...
        if (file === undefined) {
          file = { path: path, added: 0, removed: 0, diffs: [], callIds: [] };
    ...
    function GuardianSummary(props) {
      ...
      var files = computeUnkeptFiles(nodes || [], sessionId);
      ...
      var fileRows = files.map(function (file) {
        ...
        var statChildren = [];
        if (file.added > 0) statChildren.push(React.createElement("span", { key: "a", className: "dshg_add" }, "+" + file.added + "�?));
        if (file.removed > 0) statChildren.push(React.createElement("span", { key: "r", className: "dshg_rem" }, "-" + file.removed + "�?));

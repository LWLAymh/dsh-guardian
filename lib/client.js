window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var css = [
      ...
      ".dshg_resolved_badge{flex:none;display:inline-flex;align-items:center;height:18px;color:var(--dsw-alias-label-tertiary);border:1px solid var(--dsw-alias-border-l2);border-radius:9px;padding:0 6px;font-size:11px;line-height:16px}",
    ...
    function FileChangeRow(props) {
      ...
      var toolName = props.toolName || "edit";
      ...
      var callId = block && block.callId;
      useSummaryVersion(sessionId);
      var resolution = callId && sessionId ? resolvedAction(sessionId, callId) : undefined;
      ...
      if (resolution !== undefined) {
        children.push(React.createElement("span", { key: "resolved", className: "dshg_resolved_badge" }, resolution === "keep" ? "已保留" : "已撤销"));
      } else if (model !== null) {
      ...
      var row = React.createElement("div", { className: "dshg_row", "data-state": state, "data-tool": toolName }, children);
      var body = null;
      if (open && model !== null && resolution === undefined) {
    ...
    function DangerBashRow(props) {
      ...
      if (model === null) {
        ...
        return React.createElement("div", { className: "dshg_card" },
          React.createElement("div", { className: "dshg_row", "data-state": state, "data-tool": toolName },
    ...
    // ------------------------------------------------------------------
    var resolvedActionsBySession = new Map();
    var summaryListeners = new Set();

    function storageKey(sessionId) {
      return "dsh-edit-guardian:resolved:" + sessionId;
    }

    function loadResolvedMap(sessionId) {
      var map = new Map();
      try {
        var raw = window.localStorage.getItem(storageKey(sessionId));
        if (raw) {
          var parsed = JSON.parse(raw);
          for (var key of Object.keys(parsed)) {
            if (parsed[key] === "keep" || parsed[key] === "undo") {
              map.set(key, parsed[key]);
            }
          }
        }
      } catch (_) {
        // localStorage unavailable or malformed — fall back to in-memory
      }
      resolvedActionsBySession.set(sessionId, map);
      return map;
    }

    function saveResolvedMap(sessionId, map) {
      try {
        var plain = {};
        map.forEach(function (value, key) { plain[key] = value; });
        window.localStorage.setItem(storageKey(sessionId), JSON.stringify(plain));
      } catch (_) {
        // best-effort persistence
      }
    }

    function resolvedMap(sessionId) {
      var map = resolvedActionsBySession.get(sessionId);
      if (map === undefined) return loadResolvedMap(sessionId);
      return map;
    }

    function resolveCalls(sessionId, callIds, action) {
      var map = resolvedMap(sessionId);
      var changed = false;
      for (var i = 0; i < callIds.length; i++) {
        if (callIds[i] && map.get(callIds[i]) !== action) {
          map.set(callIds[i], action);
          changed = true;
        }
      }
      if (changed) {
        saveResolvedMap(sessionId, map);
        summaryListeners.forEach(function (fn) { fn(); });
      }
    }

    function resolvedAction(sessionId, callId) {
      var map = resolvedMap(sessionId);
    ...
    function computeUnkeptFiles(nodes, sessionId, reasonByCallId) {
      var resolved = resolvedMap(sessionId);
      var byPath = new Map();
      for (var i = 0; i < nodes.length; i++) {
        var node = nodes[i];
    ...
    function GuardianSummary(props) {
      ...
      function undoPath(file) {
        if (busy !== null) return;
        if (/[\r\n]/.test(file.path)) {
          setMsg("撤销失败：路径包含非法换行符");
          return;
        }
        setBusy(file.path);
        setMsg("");
        Promise.resolve(executeCommand("/undo " + file.path)).then(function (res) {
          if (res && res.error) {
            setBusy(null);
            setMsg("撤销失败：" + res.error);
            return;
          }
          resolveCalls(sessionId, file.callIds, "undo");
          setBusy(null);
      ...
      function keepPath(file) {
        if (busy !== null) return;
        resolveCalls(sessionId, file.callIds, "keep");
      ...
      function keepAll() {
        ...
        resolveCalls(sessionId, allCalls, "keep");
        setMsg("已保留全部更改");
      }
      ...
      function undoAll() {
        ...
        var allCalls = [];
        for (var i = 0; i < files.length; i++) allCalls = allCalls.concat(files[i].callIds);
        Promise.resolve(executeCommand("/undo --all")).then(function (res) {
          if (res && res.error) {
            setBusy(null);
            setMsg("全部撤销失败：" + res.error);
            return;
          }
          resolveCalls(sessionId, allCalls, "undo");
          setBusy(null);
          setMsg("已提交全部撤销");
        }).catch(function (err) {
          setBusy(null);
      ...
      ctx.slots.inject("conversation.input.dock", () => ctx.slots.register({
        ...
        inject: (sessionId) => ({
          executeCommand: function (line) {
            if (commandsRemote === undefined || commandsRemote === null) {
              return Promise.resolve({ error: "remote.commands unavailable" });
            }
            return Promise.resolve(commandsRemote.execute(sessionId, line, [])).then(function (res) {
              if (res && res.ok === false) {
                return { error: res.error && res.error.message ? res.error.message : "command failed" };
              }
              if (res && res.value && res.value.result) {
                if (res.value.result.kind === "error") {
                  return { error: res.value.result.text || "command error" };
                }
                return { ok: true, text: res.value.result.text };
              }

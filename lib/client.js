window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var css = [
      ...
      ".dshg_body{border:1px solid var(--dsw-alias-border-l1);border-radius:8px;overflow-x:hidden;overflow-y:auto;max-height:320px;margin:4px 0 4px 22px}",
    ...
    function buildReasonByCallId(nodes) {
      ...
      for (var i = 0; i < nodes.length; i++) {
        var node = nodes[i];
        if (!node || node.kind !== "assistant" || !Array.isArray(node.blocks)) continue;
        var texts = [];
    ...
    function computeUnkeptFiles(nodes, sessionId, reasonByCallId) {
      ...
      for (var i = 0; i < nodes.length; i++) {
        ...
        if (node.callId && reasonByCallId && reasonByCallId[node.callId] && file.reasons.indexOf(reasonByCallId[node.callId]) === -1) {
          file.reasons.push(reasonByCallId[node.callId]);
        }
      }
